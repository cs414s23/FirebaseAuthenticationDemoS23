...
class RegisterActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Animate the icon on the splashscreen -- optional
        myCustomAnimation(splashScreen)
    ...
    private fun myCustomAnimation(splashScreen: SplashScreen) {
        // Create an exit animation
        val rotateAnimation = RotateAnimation(
            0f, 360f, // rotate 360 degrees on X axis
        ...
        // Set the exit animation listener on the splash screen
        splashScreen.setOnExitAnimationListener { splashScreenViewProvider ->
            splashScreenViewProvider.iconView.run {
                startAnimation(rotateAnimation)
                postDelayed({ splashScreenViewProvider.remove() }, rotateAnimation.duration)
            }
        }
    }
}